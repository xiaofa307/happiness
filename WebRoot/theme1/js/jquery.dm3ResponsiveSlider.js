...
(function($) {
    ...
    function Dm3ResponsiveSlider(scroller, options) {
        ...
        this.autoscroll = parseInt(options.autoscroll, 10) * 1000;
    ...
    Dm3ResponsiveSlider.prototype = {
        ...
        init: function() {
            ...
            that.inner.bind('mousedown touchstart', function(e) {
                ...
                that.scrollFrom = parseInt(that.inner.css('marginLeft'), 10);
                that.moving = true;
            });
            ...
            that.inner.bind('mousemove touchmove', function(e) {
                ...
                if (that.curSlideNum <= 1 && that.dx < that.slideWidth * (-0.2)) {
                    return;
                } else if (that.curSlideNum >= that.numSlides && that.dx > that.slideWidth * 0.2) {
                    return;
                }
        ...
        setupControls: function() {
            ...
            this.slides.each(function(i) {
                var point = $('<a class="r-slider-point" href="#" data-slidenum="' + (i+=1) + '">' + i + '</a>');
        ...
        go: function(dir) {
            ...
            if (parseInt(this.inner.css('marginLeft'), 10) === scrollVal) {
                this.sliding = false;
                return;
            }
            ...
            if (this.points) {
                this.points.children('a').eq(this.curSlideNum - 1).removeClass('r-slider-current');
            }
            ...
            if (this.points) {
                this.points.children('a').eq(this.curSlideNum - 1).addClass('r-slider-current');
            }
        }
    ...
    $.fn.dm3ResponsiveSlider = function(options) {
        ...
        this.each(function() {
            return new Dm3ResponsiveSlider($(this), options);
        });
    };
